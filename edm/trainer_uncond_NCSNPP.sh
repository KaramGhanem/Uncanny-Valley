...
#SBATCH --gres=gpu:a100:2                                     # Ask for 2 GPU
...
#ImageNet
# torchrun --standalone --nproc_per_node=1 train.py --outdir=training-runs --data=/home/mila/k/karam.ghanem/scratch/datasets/imagenet-64x64.zip --cond=0 --arch=ncsnpp --batch-gpu=32 --snap=10 --dump=10

#CIFAR10
torchrun --standalone --nproc_per_node=1 train.py --outdir=training-runs --data=/home/mila/k/karam.ghanem/scratch/datasets/cifar10/cifar10-32x32.zip --cond=0 --arch=ncsnpp --batch=32 --snap=10 --dump=10 
